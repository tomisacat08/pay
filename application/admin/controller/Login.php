...
class Login extends Base {
    ...
    public function index() {
        ...
        //当前请求ip地址
        $ip = $this->request->ip();
        $loginIpLock = 'adminLogin:'.$username.':'.$ip;
        ...
        $userInfo = AdminUser::where('username', $username)->find();
        ...
        $return['access'] = [];
        $isSupper = Tools::isAdministrator($userId);
        if ($isSupper) {
            $access = AdminMenu::field('url')->where(['hide' => 0])->group('url')->column('url');
            $return['access'] = $access;
        } else {
            $groups = AdminAuthGroupAccess::get(['uid' => $userId]);
            if (!empty($groups) && $groups->groupId) {
                $groupIdStr = trim($groups->groupId,',');
                $access = AdminAuthRule::field('url')->whereIn('groupId', $groupIdStr)->group('url')->column('url');
                $return['access'] = $access;
            }
        }
    ...
    public function logout() {
        ...
        $update = AdminUser::where('id',$userId)->update(['user_token'=>'']);
        if($update !== false){
