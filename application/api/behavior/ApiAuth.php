...
class ApiAuth {
        ...
        if (isset($hash['rule'][1])) {
            ...
            if ($cached) {
                ...
                if ($apiInfo) {
                    ...
                    return json(['code' => ReturnCode::DB_READ_ERROR, 'msg' => '获取接口配置数据失败']);
                }
            }

            if ($this->apiInfo['accessToken']) {
    ...
    private function checkAccessToken() {
        ...
        if (!isset($access_token) || !$access_token) {
            return json(['code' => ReturnCode::ACCESS_TOKEN_TIMEOUT, 'msg' => '缺少参数access-token']);
        } else {
            $appInfo = cache('AccessToken:' . $access_token);
            if (!$appInfo) {
                return json(['code' => ReturnCode::ACCESS_TOKEN_TIMEOUT, 'msg' => 'access-token已过期']);
            }
    ...
    private function checkVersion() {
        ...
        if (!isset($version) || !$version) {
            return json(['code' => ReturnCode::EMPTY_PARAMS, 'msg' => '缺少参数version']);
        } else {
            if ($version != config('apiAdmin.APP_VERSION')) {
                return json(['code' => ReturnCode::VERSION_INVALID, 'msg' => 'API版本不匹配']);
            }
        }
    }
    ...
    private function checkLogin() {
        ...
        if ($this->apiInfo['needLogin']) {
            if (!$userToken) {
                return json(['code' => ReturnCode::AUTH_ERROR, 'msg' => '缺少user-token']);
            }
        }
        if ($userToken) {
            $userInfo = cache('user-token:' . $userToken);
            if ( !is_array($userInfo) || !isset($userInfo['user_token']) || $userInfo['user_token'] !== $userToken ) {
                return json(['code' => ReturnCode::AUTH_ERROR, 'msg' => 'user-token不匹配']);
            }
