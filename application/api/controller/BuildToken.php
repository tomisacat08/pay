...
class BuildToken extends Base {
    ...
    public function getAccessToken() {
        ...
        $this->clearOldToken($param['device_id']);
        ...
        cache('AccessToken_device:' .$param['device_id'] , $accessToken, $expires);
        $return['access_token'] = $accessToken;
        $return['expires_in'] = $expires;
        $return['app_version'] = $appInfo['app_version'];
        $return['app_update_url'] = $this->request->domain() .$appInfo['app_update_url'];
        $return['app_update_content'] = $appInfo['app_update_content'];

        return $this->buildSuccess($return);
    }


    private function clearOldToken($deviceId)
    {
        $accessToken = cache('AccessToken_device:' . $deviceId);
        if ($accessToken) {
            cache('AccessToken:' . $accessToken, null);
            cache('AccessToken_device:' . $deviceId, null);
        }
    }
