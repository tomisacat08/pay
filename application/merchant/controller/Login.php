...
class Login extends Base {
    ...
    private $waiteTime = 30; //登录错误达到限制后 n 分钟后才可以登录
    ...
    public function index() {
        ...
        //当前请求ip地址
        $ip = $this->request->ip();
        $loginIpLock = 'merchantLogin:'.$username.':'.$ip;
        ...
        $userInfo = Merchant::where('mobile', $username)->find();
    ...
    public function logout() {
        ...
        $update = Merchant::where('id',$userId)->update(['user_token'=>'']);
        if($update !== false){
    ...
    public function unlock()
    {
        $password = $this->request->post('password/s');
        if (!$password) {
            return $this->buildFailed(ReturnCode::LOGIN_ERROR, '请填写验证参数!');
        }
        $userInfo = $this->userInfo;
        $userName = $this->userInfo->mobile;
        //当前请求ip地址
        $ip = $this->request->ip();
        $loginIpLock = 'biz_'.$userName.'_'.$ip;
        $loginNum = Cache::get($loginIpLock,0);
        if($loginNum > $this->loginNum){
            return $this->buildFailed(ReturnCode::LOGIN_ERROR, '密码输错'.$this->loginNum.'次,请稍后再试');
        }
        ...
        do{
            //是否启用谷歌验证
            if($userInfo->used_google_code == 1){
                $check = GoogleService::check($userInfo->google_secret_key,$password);
                if(!$check){
                    break;
                }
            }else{
        ...
        return $this->buildSuccess($return, '解锁成功');
